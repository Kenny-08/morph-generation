...
def file_preprocess(file_name):
    ...
    pandas_holder = pd.read_csv(file_name, sep='\t', names=[
                                'word', 'inflection', 'feature'])

    words_list = [list(word) for word in list(pandas_holder['word'])]
    inflection_list = [list(inflection)
                       for inflection in list(pandas_holder['inflection'])]
    ...
    vocab_chars_inflection = [
        char for word in inflection_list for char in word]
    # print(vocab_chars_word)

    char_vocab = ['<pad>', '<sos>', '<eos>'] + \
        list(set(vocab_chars_word+vocab_chars_inflection))
    feat_vocab = ['<pad>'] + \
        list(set([f for feat in feature_list for f in feat]))
...
def mini_batch_creator(sentence_list, batch_size):
    final = [sentence_list[i * batch_size:(i + 1) * batch_size] for i in range(
        (len(sentence_list) + batch_size - 1) // batch_size)]
    return final
...
class EncoderLSTM(nn.Module):
    def __init__(self, vocab_size, embedding_size, encoder_hid_dem, decoder_hid_dem, bidirectional, device):
        ...
        self.embedding_layer = nn.Embedding(
            num_embeddings=vocab_size, embedding_dim=embedding_size, padding_idx=0)
        self.GRU_layer = nn.GRU(input_size=embedding_size, hidden_size=encoder_hid_dem,
                                batch_first=True, bidirectional=bidirectional)
        self.fc = nn.Linear(
            encoder_hid_dem*self.encoder_n_direction, decoder_hid_dem)
...
class Attention(nn.Module):
    def __init__(self, encoder_hid_dem, decoder_hid_dem, bidirectional):
        ...
        if(bidirectional == True):
            self.encoder_n_direction = 2
        self.attn = nn.Linear(
            (encoder_hid_dem * self.encoder_n_direction) + decoder_hid_dem, decoder_hid_dem)
...
class DecoderLSTM(nn.Module):
    def __init__(self, decoder_hid_dem, encoder_hid_dem, vocab_size, embedding_dim, attention, decoder_input_size, linear_input_size, bidirectional, device):
        ...
        self.out_layer = nn.Linear(
            in_features=linear_input_size, out_features=vocab_size)
    ...
    def forward(self, feature, hidden, actual_word, encoder_outputs):
        ...
        output = torch.cat(
            (output.squeeze(0), weighted.squeeze(1), actual_word.squeeze(1)), dim=1)
...
def train_model(encoder, decoder, encoder_optimizer, decoder_optimizer, criteria, train_input_word, train_output_word, train_feature_words, max_word_length, device):
    ...
    actual_word = encoder.embedding_layer(torch.tensor(
        char_to_index['<sos>']).view(1, -1).to(device)).repeat(batch_size, 1, 1)
    ...
    for idx in range(max_word_length):
        ...
        predicted_output, hidden = decoder(
            feature, hidden, actual_word, encoder_outputs)
        ...
        for row in range(bs):
            ...
            temp = encoder.embedding_layer(
                torch.tensor(index).view(1, -1).to(device))
            temp2 = torch.cat((temp2, temp))

        teacher_force = random.random() < .5

        if teacher_force == 1:
            actual_word = encoder.embedding_layer(
                train_output_word[:, idx]).unsqueeze(1)
        else:
            actual_word = temp2
...
def validate_model(encoder, decoder, criteria, valid_input_word, valid_output_word, valid_feature_words, max_word_length, device):
    ...
    initial_word = encoder.embedding_layer(torch.tensor(
        char_to_index['<sos>']).view(1, -1).to(device)).repeat(batch_size, 1, 1)
    ...
    for idx in range(max_word_length):
        ...
        predicted_output, hidden = decoder(
            feature, hidden, initial_word, encoder_outputs)
...
def train_iter(encoder, decoder, input_word, output_word, feat_encode_list, valid_word, valid_label, valid_feat, device, encoder_optimizer, decoder_optimizer, batch_size, epoch_size):
    ...
    input_word_minibatches = mini_batch_creator(
        input_word, batch_size=batch_size)
    output_word_minibatches = mini_batch_creator(
        output_word, batch_size=batch_size)
    feature_minibatches = mini_batch_creator(
        feat_encode_list, batch_size=batch_size)
    ...
    valid_word, valid_label, max_length_1 = source_taget_equal_length_maker(
        valid_word, valid_label)
    ...
    for epoch in range(epoch_size):
        ...
        for batch_idx, _ in enumerate(input_word_minibatches):
            ...
            train_word, train_label, max_length = source_taget_equal_length_maker(
                train_word, train_label)
            ...
            loss, accuracy = train_model(encoder, decoder, encoder_optimizer, decoder_optimizer,
                                         criteria, train_word, train_label, train_feat, max_length, device)
        ...
        val_loss, val_accuracy = validate_model(
            encoder, decoder, criteria, valid_word, valid_label, valid_feat, max_length_1, device)
...
feat_encode_list = sentence_pad(sentence_encoder(
    feature_list, feature_to_index), max_feature_length)


test_words_list, test_inflection_list, test_feature_list, _, _, _, _, _, _ = file_preprocess(
    evaluate_file)
...
test_feat_encode_list = sentence_pad(sentence_encoder(
    test_feature_list, feature_to_index), max_feature_length)
...
if(encoder_bidirectional):
    decoder_input_size = len(
        feat_encode_list[0]) + embedding_size+(encoder_hid_dem)*2
    linear_input_size = embedding_size+decoder_hid_dem+(encoder_hid_dem)*2
else:
    decoder_input_size = len(
        feat_encode_list[0]) + embedding_size+(encoder_hid_dem)*1
...
class Seq2Seq:
    def __init__(self, vocab_size, embedding_size, encoder_hid_dem, decoder_hid_dem, device, encoder_bidirectional, decoder_input_size, linear_input_size, decoder_birectional, lr):
        self.encoder = EncoderLSTM(vocab_size, embedding_size, encoder_hid_dem, decoder_hid_dem,
                                   encoder_bidirectional, device).to(device)              # (58, 300, 150)
        self.attention = Attention(
            encoder_hid_dem, decoder_hid_dem, encoder_bidirectional)
        self.decoder = DecoderLSTM(decoder_hid_dem, encoder_hid_dem, vocab_size, embedding_size, self.attention,
                                   decoder_input_size, linear_input_size, decoder_birectional, device).to(device)          # (606, 300, 58)
        self.encoder_optimizer = torch.optim.Adam(
            self.encoder.parameters(), lr)
        self.decoder_optimizer = torch.optim.Adam(
            self.decoder.parameters(), lr)

    def train(self):
        train_acc, train_loss, val_acc, val_loss = train_iter(self.encoder, self.decoder, word_encode_list, inf_encode_list, feat_encode_list, test_word_encode_list,
                                                              test_inf_encode_list, test_feat_encode_list, device, self.encoder_optimizer, self.decoder_optimizer, batch_size=BATCH_SIZE, epoch_size=EPOCH_SIZE)
        ploatter(loss=train_loss, acc=train_acc,
    ...
    def test_model(self, test_input_word, test_feature_words, inflection_encode_list1, device, max_word_length=25):
        ...
        initial_word = self.encoder.embedding_layer(torch.tensor(
            char_to_index['<sos>']).view(1, -1).to(device)).repeat(batch_size, 1, 1)
        ...
        for idx in range(max_word_length):
            predicted_output, hidden = self.decoder(
                feature, hidden, initial_word, encoder_outputs)
            topv, topi = predicted_output.topk(1)
            bs = topi.size()[0]
            temp2 = torch.zeros(0, 1, 300).to(device)
            ...
            for row in range(bs):
                index = topi[row][0].item()
                act[row][idx] = index
                temp = self.encoder.embedding_layer(
                    torch.tensor(index).view(1, -1).to(device))
                temp2 = torch.cat((temp2, temp))
            ...
            num_correct = (predicted_label ==
                           inflection_encode_list1[:, idx]).sum()
            vali_running_acc += num_correct.item()
...
s2s = Seq2Seq(vocab_size, embedding_size, encoder_hid_dem, decoder_hid_dem, device,
              encoder_bidirectional, decoder_input_size, linear_input_size, decoder_birectional, learning_rate)
...
test_words_list, test_inflection_lists, test_feature_list, _, _, _, _, _, _ = file_preprocess(
    test_file)
...
feat_encode_list1 = sentence_pad(sentence_encoder(
    test_feature_list, feature_to_index), max_feature_length)
...
for index in range(0, len(word_encode_list1)):
    ...
    test_word, test_label, max_len = source_taget_equal_length_maker(
    ...
    predic_out, ac = s2s.test_model(
        test_word, test_feat, test_label, device, max_len)
